...
with arcpy.da.SearchCursor(DyadTable,DyadTable_FieldList) as cursor:
	for row in cursor:
		if row[DyadTable_FieldList.index(DyadRec_field)] == row[DyadTable_FieldList.index(DyadProv_field)] and row[DyadTable_FieldList.index("VISITS_DYAD")] == row[DyadTable_FieldList.index("MAX_VISITS")]:
...
for i in seed_List:
	...
	with arcpy.da.SearchCursor(DyadTable,DyadTable_FieldList,dyadQuery) as cursor:
		for row in cursor:
			if str(row[DyadTable_FieldList.index(DyadRec_field)]) in temp_nbr_List and row[DyadTable_FieldList.index("MAX_VISITS")] != 1:
...
while len(unAssigned_List) > 0:
	...
	for i in unAssigned_List:
		...
		with arcpy.da.SearchCursor(NeighborTable,NBRTable_FieldList,ZCTAQuery) as cursor:
			for row in cursor:
				# if nbr has been assigned, append to the temp list as candidate
				...
				with arcpy.da.SearchCursor(DyadTable,DyadTable_FieldList,dyadQuery) as cursor:
					for row in cursor:
						...
						if str(row[DyadTable_FieldList.index(DyadProv_field)]) in temp_key_List:
							if row[DyadTable_FieldList.index("MAX_VISITS")] == 1:
