...
# Version		: $1.0$
# Description	:Creates a dyad table from geocoded points of members. Members must have a provider
# zip identified
#--------------------------------------------------------------------------------------------------
...
with arcpy.da.InsertCursor(dyadTable,table_ls) as cursor:
	for key, ls in memDict.iteritems(): #pull each member zip from member dict
		...
		for k, v in temp.iteritems(): #iterate through temp dictionary
			#insert the mem zip, the prov zip (k), the value , max visits and total visits
...
###################################################################################################
arcpy.AddMessage('New Dyad Table Name: {0}\nLocation: {1}'.format(tableName, outputPath))
